#!/usr/bin/env python
"""Thomas Liao Oct 2018 wrapper class for TripletNet for ease of use.
"""
...
class TripletEmbedding:
    def __init__(self, weight_fname='/work_12t/tliao4/tracking/diva_scripts_jin/triplet_reid/trinet-mars.npz',
        ...
        resnet = self._build_model(input_size=(None, 3, 256, 128))
        ...
        # Set the parameters
        with np.load(self.weight_fname) as f:
    ...
    def _get_augmentation_batch(self, image, im_mean):
    ...
    def _build_simple_block(self, incoming_layer, names,
    ...
    def _build_residual_block(self, incoming_layer, ratio_n_filter=1.0, ratio_size=1.0, has_left_branch=False,
                              upscale_factor=4, ix=''):
        ...
        # right branch
        net_tmp, last_layer_name = self._build_simple_block(
    ...
    def _build_model(self, input_size):
        ...
        sub_net, parent_layer_name = self._build_simple_block(
        ...
        for c in block_size:
            if c == 'a':
                sub_net, parent_layer_name = self._build_residual_block(net[parent_layer_name], 1, 1, True, 4,
                                                                        ix='2%s' % c)
            else:
                sub_net, parent_layer_name = self._build_residual_block(net[parent_layer_name], 1.0 / 4, 1, False, 4,
                                                                        ix='2%s' % c)
            net.update(sub_net)
        ...
        for c in block_size:
            if c == 'a':
                sub_net, parent_layer_name = self._build_residual_block(
    ...
    def triplet_net_embedded_output(self, image):
        """ TripletNet embedding of cropped human. Single img input has been tested and working. Batch input not yet.
        Args: img: (cropped) img of diva walking single person
        Returns:
        """
        batch = self._get_augmentation_batch(image, self.im_mean)
        # convert to naive python list?
        embedding = np.mean(self.predict_features(batch), axis=0).tolist()
...
if __name__ == "__main__":
    # pdb.set_trace()
    tri_emb = TripletEmbedding()
    test_img = cv2.imread('/work_12t/tliao4/tracking/diva_scripts_jin/triplet_reid/test_imgs/1-1.jpg')
    print((tri_emb.triplet_net_embedded_output(test_img)))
